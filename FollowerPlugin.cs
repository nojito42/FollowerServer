    ...
    private void ManageLeaderOnSameMap()
    {
        ...
        if (leaderEntity != null)
        {
            ...
            if (Settings.PartySubMenu.UseCriesAuto && leaderEntity.DistancePlayer < 20 && (GameController.Area.CurrentArea.IsHideout == false && GameController.Area.CurrentArea.IsTown == false))
            {
                ...
                {
                    if(GameController.Player.Buffs.Any(b=> b.Timer > 0 || b.DisplayName.Contains(crySkill.Name)))
                    {
...
//public class FollowerPlugin2 : BaseSettingsPlugin<FollowerPluginSettings>
//{
//    public List<PlayerSkill> FollowerSkills = [];
//    public Entity LastTargetedPortalOrTransition { get; set; } = null;
//    public Leader Leader { get; private set; }
